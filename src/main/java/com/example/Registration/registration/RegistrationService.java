...
import com.example.Registration.email.EmailValidator;
...
public class RegistrationService {
    ...
    public String register(RegistrationRequest request) {
        ...
        AppUser appUser = appUserService.signUpAppUser(new AppUser(request.getFirst_name(), request.getLast_name(), request.getEmail(), request.getPassword(), AppUserRole.USER));
        Optional<ConfirmationToken> token = confirmationTokenService.getTokenByAppUserId(appUser.getId());

        String link = "http://localhost:8080/api/registration/confirm?token="+token.get().getToken();

        emailSender.send(request.getEmail(), buildEmail((request.getFirst_name()+" "+request.getLast_name()),link));
